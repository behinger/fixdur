...
def contrast(inmat, ws = 51): # ws has to be positive odd
    sq = sameMat(inmat)
    smth = sameMat(inmat)
    outmat = sameMat(inmat)
    cv.Mul(inmat, inmat, sq)
    cv.Smooth(sq, sq, cv.CV_GAUSSIAN, ws, ws) # width & height of the kernel
    cv.Smooth(inmat, smth, cv.CV_GAUSSIAN, ws, ws)
    cv.Mul(smth, smth, smth)
    cv.Sub(sq, smth, outmat)
    cv.Normalize(outmat, outmat, 0, 1, cv.CV_MINMAX) # scale & shift to min=0 max=1
    cv.Pow(outmat, outmat, 0.5)
    cv.Threshold(outmat, outmat, 0, 0, cv.CV_THRESH_TOZERO) # unnecessary as no negative value
    del smth, sq
    return outmat
...
def sobel(inmat, ws = 7): # the common way to apply Sobel operator
    h = sameMat(inmat)
    v = sameMat(inmat)
    outmat = sameMat(inmat)
    ...
    cv.Mul(v, v, v)
    cv.Add(h, v, outmat)
    # what about cv.Normalize(outmat, outmat, 0, 1, cv.CV_MINMAX) ?
    cv.Pow(outmat, outmat, 0.5)
    return outmat
