...
def paths():
    ...
    if os.path.exists('/home_local/tracking/experiments/fixdur_gist/'):
        path_to_fixdur_files = '/home_local/tracking/experiments/fixdur_gist/'
        path_to_fixdur_code = '/home_local/tracking/experiments/fixdur_gist/expcode/'
...
def randomization(subject, trial_time):
    ...
    for b in range(int(len(images)/2)):
        imgt.append(1)
        imgt.append(0)
    random.shuffle(imgt)
    ...
    for k,image in enumerate(images):
        ...
        while time<trial_time:
            ...
            imgt_a=np.append(imgt_a,imgt[k])
    ...
    imgt_a = np.reshape(imgt_a,(len(imgt_a),1))    
    ...
    trial_mat = np.append(trial_mat,imgt_a,axis=1)
...
def full_image_load(surf,current_trial,path_to_fixdur_files,noise):
         #print path_to_fixdur_files+'stimuli/noise/'+noise

         #print current_trial
         if current_trial[0,4] =='1.0':
                #print 'fullTrial'
                stim = pygame.image.load(path_to_fixdur_files+'stimuli/noise/'+noise).convert()
                surf.blit(stim,(320,60))
                pygame.display.update()
