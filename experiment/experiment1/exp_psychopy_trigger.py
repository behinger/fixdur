...
#from eyelink_psychopy_be import EyeLinkCoreGraphicsOpenGL
#import ioHUBeyelink as iohu
...
# set up the window
rectXY = (1920,1080);
surf = visual.Window(size=rectXY,fullscr=False,winType = 'pygame')
...
for chosen_image in range(NUM_OF_TRIALS-START_TRIAL):
    ...
    if current_trial[0][1] == 'seq':
        ...
        for bubble in remaining_bubbles:
            x,y = tools.get_bubble_pos(bubble)
            #loaded_bubbles.append(pygame.image.load(path_to_fixdur_files+'stimuli/single_bubble_images/'+bubble_image+'/'+bubble).convert_alpha())
            loaded_bubbles.append(visual.SimpleImageStim(surf, image = path_to_fixdur_files+'stimuli/single_bubble_images/'+bubble_image+'/'+bubble, pos = (-200,0)))            
            loaded_bubbles2.append(visual.SimpleImageStim(surf, image = path_to_fixdur_files+'stimuli/single_bubble_images/'+bubble_image+'/'+bubble, pos = (200,0)))            
            all_bubbles.append([int(bubble.split('_',1)[1].split('_')[0]),int(bubble.split('_',1)[1].split('_')[1].split('.')[0])])
        remaining_bubbles = list(all_bubbles)
        ...
        for subtrial in current_trial:
            ...
            if (np.mod(subtrial_num,2) == 1):
                ...
                used_bubble, more_bubbles = trial.get_image(surf,bubble_image,all_bubbles,loaded_bubbles2,remaining_bubbles,chosen_bubble,num_of_bubbles,dist_mat) 
